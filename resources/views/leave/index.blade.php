...
@section('content')
<div x-data="{ applyLeaveModal: false }">
    ...
    <div class="flex justify-between items-start mb-8">
        ...
        <button @click="applyLeaveModal = true" class="bg-[#22C55E] hover:bg-emerald-600 text-white px-5 py-2.5 rounded-lg text-sm font-bold flex items-center gap-2 transition shadow-sm cursor-pointer">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden p-6">
        ...
        <table class="w-full text-left border-collapse">
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <tr class="hover:bg-slate-50 transition">
                    <td class="py-4 text-sm font-bold text-slate-900">{{ $emp->first_name }} {{ $emp->last_name }}</td>
                    <td class="py-4">
                        <span class="text-sm font-bold {{ $emp->vacation_balance <= 0 ? 'text-red-500' : 'text-[#16A34A]' }}">{{ $emp->vacation_balance }} days</span>
                    </td>
                    <td class="py-4">
                        <span class="text-sm font-bold {{ $emp->sick_balance <= 0 ? 'text-red-500' : 'text-[#F97316]' }}">{{ $emp->sick_balance }} days</span>
                    </td>
                    <td class="py-4">
                        <span class="text-sm font-bold {{ $emp->emergency_balance <= 0 ? 'text-red-500' : 'text-[#64748B]' }}">{{ $emp->emergency_balance }} days</span>
                    </td>
                </tr>
                @endforeach
            </tbody>
    ...
    <div x-show="applyLeaveModal" class="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4" style="display: none;">
        <div @click.away="applyLeaveModal = false" class="bg-white rounded-xl shadow-2xl border border-slate-200 w-full max-w-lg overflow-hidden">
            
            <div class="bg-[#0B1C3D] p-6 text-white flex justify-between items-center">
                <h3 class="text-lg font-bold">File Leave on Behalf of Employee</h3>
                <button @click="applyLeaveModal = false" class="text-white/50 hover:text-white transition">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                </button>
            </div>

            <form action="{{ route('hr.leave.store') }}" method="POST" class="p-6 space-y-4" x-data="{ selectedType: 'Vacation Leave' }">
                @csrf
                
                <div>
                    <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Select Employee</label>
                    <select name="employee_id" class="w-full border border-slate-300 rounded-lg p-2.5 text-sm focus:border-[#15A34A] focus:outline-none cursor-pointer" required>
                        <option value="" disabled selected>Choose an employee...</option>
                        @foreach($employees as $emp)
                            <option value="{{ $emp->id }}">{{ $emp->first_name }} {{ $emp->last_name }} (VL: {{ $emp->vacation_balance }}, SL: {{ $emp->sick_balance }})</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select name="leave_type" x-model="selectedType" class="w-full border border-slate-300 rounded-lg p-2.5 text-sm focus:border-[#15A34A] focus:outline-none cursor-pointer" required>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="date" name="start_date" 
                               ...
                               class="w-full border border-slate-300 rounded-lg p-2.5 text-sm focus:border-[#15A34A] focus:outline-none cursor-pointer" required>
                    </div>
                    <div>
                ...
                <div>
                    ...
                    <textarea name="reason" rows="3" class="w-full border border-slate-300 rounded-lg p-2.5 text-sm focus:border-[#15A34A] focus:outline-none" required></textarea>
                </div>

                <div class="pt-4 border-t border-slate-100 flex justify-end gap-3">
                    <button type="button" @click="applyLeaveModal = false" class="px-5 py-2.5 rounded-lg text-sm font-bold border border-slate-300 text-slate-700 hover:bg-slate-50 transition">Cancel</button>
                    <button type="submit" class="px-5 py-2.5 rounded-lg text-sm font-bold bg-[#15A34A] hover:bg-emerald-600 text-white transition">Apply & Deduct Balance</button>
                </div>
            </form>
        </div>
    </div>
