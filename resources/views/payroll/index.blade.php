...
<div class="bg-white rounded-xl shadow-sm border border-slate-200 p-4 mb-6 flex justify-between items-center">
    ...
    <div class="flex gap-3">
        <button class="bg-white border border-slate-300 text-slate-700 px-4 py-2 rounded-lg text-sm font-bold flex items-center gap-2 hover:bg-slate-50 transition">
...
<div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6 flex flex-col justify-between h-32">
        ...
        <span class="text-2xl font-black text-slate-900">{{ $summary['gross_pay'] }}</span>
    </div>
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6 flex flex-col justify-between h-32">
        <span class="text-slate-500 text-sm font-bold">Total Deductions</span>
        <span class="text-2xl font-black text-[#EF4444]">{{ $summary['deductions'] }}</span>
    </div>
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6 flex flex-col justify-between h-32">
        <span class="text-slate-500 text-sm font-bold">Total Net Pay</span>
        <span class="text-2xl font-black text-[#22C55E]">{{ $summary['net_pay'] }}</span>
    </div>
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6 flex flex-col justify-between h-32">
        <span class="text-slate-500 text-sm font-bold">Employees Paid</span>
        <span class="text-2xl font-black text-slate-900">{{ $summary['employees'] }}</span>
    </div>
</div>
        ...
        <table class="w-full text-left border-collapse">
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <tr class="hover:bg-slate-50 transition">
                    ...
                    <td class="py-4 px-6 text-sm font-medium text-slate-700 text-right">{{ $pay['gross'] }}</td>
                    <td class="py-4 px-6 text-sm font-medium text-[#EF4444] text-right">{{ $pay['sss'] }}</td>
                    <td class="py-4 px-6 text-sm font-medium text-[#EF4444] text-right">{{ $pay['philhealth'] }}</td>
                    <td class="py-4 px-6 text-sm font-medium text-[#EF4444] text-right">{{ $pay['pagibig'] }}</td>
                    <td class="py-4 px-6 text-sm font-medium text-[#EF4444] text-right">{{ $pay['tax'] }}</td>
                    <td class="py-4 px-6 text-sm font-bold text-[#22C55E] text-right">{{ $pay['net'] }}</td>
                </tr>
                @endforeach
            </tbody>
            <!-- Totals Footer -->
            <tfoot>
                <tr class="bg-slate-50 border-t-2 border-slate-200">
                    ...
                    <td class="py-4 px-6 text-sm font-extrabold text-slate-900 text-right">{{ $totals['gross'] }}</td>
                    <td class="py-4 px-6 text-sm font-extrabold text-[#EF4444] text-right">{{ $totals['sss'] }}</td>
                    <td class="py-4 px-6 text-sm font-extrabold text-[#EF4444] text-right">{{ $totals['philhealth'] }}</td>
                    <td class="py-4 px-6 text-sm font-extrabold text-[#EF4444] text-right">{{ $totals['pagibig'] }}</td>
                    <td class="py-4 px-6 text-sm font-extrabold text-[#EF4444] text-right">{{ $totals['tax'] }}</td>
                    <td class="py-4 px-6 text-sm font-extrabold text-[#22C55E] text-right">{{ $totals['net'] }}</td>
                </tr>
            </tfoot>
        </table>
    </div>
...
<div class="flex justify-end items-center gap-6">
    ...
    <button class="bg-[#22C55E] hover:bg-emerald-600 text-white px-6 py-2.5 rounded-lg text-sm font-bold flex items-center gap-2 transition shadow-sm">
